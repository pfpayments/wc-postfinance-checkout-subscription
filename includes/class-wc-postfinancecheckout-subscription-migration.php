...
/**
 *
 * WC_PostFinanceCheckout_Subscription_Migration Class
 *
 * PostFinanceCheckout
 ...
 */
class WC_PostFinanceCheckout_Subscription_Migration {

	const CK_DB_VERSION = 'wc_postfinancecheckout_subscription_db_version';
	...
	public static function install_db( $networkwide ) {
		...
		if ( ! defined( 'WC_POSTFINANCECHECKOUT_SUBSCRIPTION_INSTALLING' ) ) {
			define( 'WC_POSTFINANCECHECKOUT_SUBSCRIPTION_INSTALLING', true );
		}
	...
	private static function check_requirements() {
		...
		if ( ! is_plugin_active( $plugin_base_name ) ) {
			...
			if ( version_compare( $base_module_data['Version'], WC_POSTFINANCECHECKOUT_SUBSCRIPTION_REQUIRED_POSTFINANCECHECKOUT_VERSION, '<' ) ) {
					/* translators: %s is replaced with "string" */
				$errors[] = sprintf( __( "PostFinance Checkout %1\$s+ is required. (You're running version %2\$s)", 'woo-postfinancecheckout-subscription' ), WC_POSTFINANCECHECKOUT_SUBSCRIPTION_REQUIRED_POSTFINANCECHECKOUT_VERSION, $base_module_data['Version'] );
			}
		}

		if ( ! is_plugin_active( 'woocommerce-subscriptions/woocommerce-subscriptions.php' ) ) {
				/* translators: %s is replaced with "string" */
			$errors[] = sprintf( __( 'Subscriptions %s+ has to be active.', 'woo-postfinancecheckout-subscription' ), WC_POSTFINANCECHECKOUT_REQUIRED_WC_SUBSCRIPTION_VERSION );
		} else {
			$woocommerce_subscriptions_data = get_plugin_data( WP_PLUGIN_DIR . '/woocommerce/woocommerce.php', false, false );

			if ( version_compare( $woocommerce_subscriptions_data['Version'], WC_POSTFINANCECHECKOUT_REQUIRED_WC_SUBSCRIPTION_VERSION, '<' ) ) {
				$errors[] = sprintf( __( "Subscriptions %1\$s+ is required. (You're running version %2\$s)", 'woo-postfinancecheckout-subscription' ), WC_POSTFINANCECHECKOUT_REQUIRED_WC_SUBSCRIPTION_VERSION, $woocommerce_subscriptions_data['Version'] );
			}
		}

		if ( ! empty( $errors ) ) {
			$title = __( 'Could not activate plugin PostFinance Checkout Subscription', 'woo-postfinancecheckout-subscription' );
			$message = '<h1><strong>' . $title . '</strong></h1><br/>' .
					'<h3>' . __( 'Please check the following requirements before activating:', 'woo-postfinancecheckout-subscription' ) . '</h3>' .
					'<ul><li>' .
	...
	public static function get_base_plugin_name() {
		...
		foreach ($plugins as $plugin) {
			if (substr_count($plugin, 'woocommerce-postfinancecheckout.php')) {
				$base_name = $plugin;
				break;
			}
		}
...
WC_PostFinanceCheckout_Subscription_Migration::init();
